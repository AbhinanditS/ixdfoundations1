...
let gameOver = false;
let startTime;
let timeLimit = 15; // Initial time limit is 15 seconds
let goal = 5; // Collect 5 blue, 5 yellow, and 5 red obstacles

function setup() {
  createCanvas(600, 600); // Canvas size is 600x800
  car = new Car();
...
function draw() {
  ...
  if (!gameOver) {
    ...
    for (let i = blueObstacles.length - 1; i >= 0; i--) {
      ...
      if (car.hits(blueObstacles[i])) {
        ...
        if (score >= goal) {
          ...
          startTime = frameCount;
          goal += 5; // Increase the goal for the next round
          timeLimit = 15; // Reset the time limit for the next round
        }
      }

      if (blueObstacles[i].offscreen()) {
        blueObstacles.splice(i, 1);
      }
    }
    ...
    for (let i = yellowObstacles.length - 1; i >= 0; i--) {
      ...
      if (yellowObstacles[i].offscreen()) {
        yellowObstacles.splice(i, 1);
      }
    }
